...
## 현재 상태 (2026-08-17 기준)
- **v9(versionCode 9, 1.2.3) 빌드 완료 — 아직 어디에도 올리지 않음**(2026-08-22). 커밋 `c8833e2` 기준.
...
- **프로덕션 초안이 게시 개요에 저장돼 있다(제출 전)**: 번들 versionCode 8 · 국가 대한민국 · 전체 출시 100% · 출시 노트 ko-KR.
  v9로 내보내려면 이 초안의 번들을 8→9로 교체해야 한다. `.aab` 업로드는 파일 크기(45MB) 때문에 Leo가 직접 붙여야 한다.
...
- 다음 할 일: ① v9 `.aab` 업로드 → ② 프로덕션 초안 번들 교체 → ③ 검토 제출 → ④ 공개 후 비공개 테스트 종료 → ⑤ JSONBin 계정 삭제 회신 대기.
