...
- **v5(versionCode 5, 1.1.0) 비공개 테스트 트랙에 제출 완료 — Google 검토 중.** 통과 시 테스터에게 자동 배포(관리형 게시 꺼짐). 내용: 정산 정확도 수정 + 암호화 공유.
- **데이터 보안 신고도 수정 제출 완료**(수집·공유 예 / 이름·기타 금융 정보 / 전송 중 암호화 / 삭제 요청 가능). 이전의 "수집·공유 없음" 불일치는 해소됨.
- Play 14일 요건: 콘솔 실측 **8일 경과 · 테스터 12명 유지** → **8/10 전후 충족 예정**. "프로덕션 신청" 버튼 아직 비활성.
- 참여 링크 `https://play.google.com/apps/testing/com.leoinkang.trippay`
- 다음 할 일: ① v5 배포 후 **실기기 검증**(공유 생성→카톡→링크 열림→공유 취소 — 취소 확인창은 웹에서 못 눌러본 유일한 부분) → ② 8/10 전후 프로덕션 신청 → ③ (여유 시) JSONBin 계정 해지.
