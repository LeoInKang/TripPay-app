...
## 현재 상태 (2026-08-17 기준)
- **v9(versionCode 9, 1.2.3) 프로덕션 트랙에 검토 제출 완료 — Google 검토 중**(2026-08-22). 커밋 `c8833e2` 기준.
  **첫 프로덕션 출시라 검토가 며칠 걸릴 수 있다**(업데이트 검토는 몇 시간). 통과 즉시 일반 공개된다.
...
- **비공개 테스트 트랙은 프로덕션 공개 후 종료하기로 했다** — 업데이트를 여러 번 해도 피드백이 없어 유지 이유가 사라졌다.
  순서를 지킬 것: 프로덕션 공개 확인 → 그다음 종료. 먼저 닫으면 검토 대기 동안 테스터 20명이 재설치할 방법이 없다.
- **내부 테스트 트랙 설정 완료**(2026-08-22). 테스터 목록 `Internal - Leo` = fompy98@gmail.com 1명.
  **버전은 일부러 안 올렸다** — 프로덕션 v9 검토 중에 올리면 게시 개요에 다른 변경사항이 끼어든다. 검토가 끝난 뒤 올린다.
  앞으로의 흐름: 빌드 → 내부 테스트로 본인 기기 확인(검토 대기 거의 없음) → 같은 번들을 프로덕션으로 승격.
  비공개 테스트와 달리 테스터 12명·14일 같은 조건이 없다. 참여 링크는 트랙 '테스터' 탭의 '링크 복사'에서 얻는다.
- 다음 할 일: ① 프로덕션 검토 결과 확인 → ② 공개 후 비공개 테스트 트랙 종료 → ③ 내부 테스트에 v9 올려 기준선 만들기 → ④ JSONBin 계정 삭제 회신 대기.
...
- **2026-08-08 잔여 정리**: 앱·뷰어·yakizy 전부 JSONBin 참조 0건 확인. Access Key 목록 비어 있음. Master Key는 재발급해 옛 값 무효화(401 실측).
  ...
  **2026-08-20 삭제 요청 메일 발송**(jsonbin.app@gmail.com, 계정 ID·등록 메일 기재). 회신 대기 중.
