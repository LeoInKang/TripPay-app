...
## 현재 상태 (2026-08-17 기준)
- **v8(versionCode 8, 1.2.2) 비공개 테스트 트랙에 제출 완료 — Google 검토 중**(2026-08-07 저녁). 커밋 `64f5eb1` 기준.
...
- 다음 할 일: ① 프로덕션 승인 메일 확인 → ② 승인 시 프로덕션 출시 여부 결정 → ③ JSONBin 계정 삭제 회신 대기.
...
9. 정산 로직을 고쳤으면 **`node test-settle.mjs` 통과 필수**(현재 67/67).
