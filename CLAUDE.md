...
- **스토어에 보이는 이메일은 `trippay.app@gmail.com`**(2026-09-13 교체). 실명 계정 주소를 스토어에서 걷어내려고 만든 문의 전용 계정이다.
  바꾼 곳은 두 군데 — 스토어 설정 「스토어 등록정보 연락처 세부정보」 · 개발자 계정 「개발자 프로필에 포함된 세부정보 → 개발자 이메일」(인증 코드 확인 필요했다).
  「Google에서 연락하는 방법」의 연락처 이메일은 **`fompy98` 그대로** 둔다 — 스토어에 안 나오고 구글 통보가 오는 곳이다.
  문의는 Leo 아이폰 기본 메일 앱에 계정을 추가해 받는다. Gmail 전달은 쓰지 않는다(답장이 `fompy98`로 나가 실명 계정이 드러난다).
...
## 현재 상태 (2026-09-13 기준)
- **사용자 추이**(2026-09-13 확인, 통계는 약 3일 늦다): 설치한 사용자 수 8월 중순 14~15명 → **9/9 20명**(최고 22명).
  최근 28일 기기 획득 6(이전 28일 1) · 처음 실행 4 · 월간 활성 기기 14(+40%) · 평점 5.00. 전부 한국.
  오름세가 v13 프로덕션 출시 무렵과 겹치지만 인과는 이 숫자로 알 수 없다.
- **Android 개발자 인증 등록 완료**(2026-07-19, 패키지 1개·서명 키 1개 「등록됨」). 9/9에 온
  「2026년 9월 30일까지 등록하세요」 알림은 전체 공지라 해당 없다. **새 앱을 만들거나 서명 키를 바꾸면 다시 등록해야 한다.**
- **★ v13(versionCode 13, 1.2.7) 프로덕션 출시 완료**(2026-08-31, 제출 다음 날 통과). 큰누나 추가 피드백이 없어 승격했다.
  스토어에 반영됨. 기존 사용자 17명은 v9 → v13으로 한 번에 올라간다.
  내부 테스트에서 **「버전 승급 → 프로덕션」**으로 넘겼다 — 파일 재업로드 없이 번들이 그대로 실린다.
  프로덕션은 내부 테스트와 달리 **저장 후 「게시 개요」에서 한 번 더 제출**해야 검토에 들어간다.
  출시 노트는 승급 시 내부 테스트 것을 그대로 물고 오므로 **다시 써야 한다** — 프로덕션 사용자는
  v9에서 건너뛰기 때문에 다통화 자체가 처음이다.
  프로덕션 직전 버전은 v9(1.2.3), 설치 17회.
- v13(versionCode 13, 1.2.7) 내부 테스트 게시(2026-08-30 오후 9:24). 커밋 `08c4975` 기준.
...
- 다음 할 일: ① JSONBin 계정 삭제 회신 대기 → ② Android vitals·리뷰·`trippay.app` 문의 모니터링. (큰누나 피드백·프로덕션 승격은 v13으로 완료)
